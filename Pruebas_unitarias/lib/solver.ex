  ...
  def words do
    "./resources/words.txt"
      ...
      def doublets_impl(word_seqs, word_2) do
        IO.inspect(word_seqs, label1: "imput")
        ...
        def find_variants(word) do
          word
          |> same_length_words(word) do
          |>Enum.filter(fn w -Z distance(word, w) == 1 end)
        end
        ...
        letters1 = String.graphemes(word_1)
        letters2 = String.graphemes(word_2)

        Enum.zip_with(letters1, letters2, fn a, b -> a == b end)
        |>  Enum.count(&(not &1))
      end
